...
from ef_gpu.readiness import write_circuit_corpus_readiness, write_sprint3_corpus_readiness
...
def main() -> int:
    ...
    external_corpus.add_argument("--learn-fpga-source", type=Path, required=True, help="learn-fpga checkout at the registered commit")
    external_corpus.add_argument("--fma-rtl-source", type=Path, required=True, help="Tachyum FMA RTL checkout at the registered commit")
    ...
    sprint3_readiness = subcommands.add_parser("sprint3-corpus-readiness", help="report the Sprint 3 external-corpus gate")
    sprint3_readiness.add_argument("source", type=Path, help="validated circuit corpus JSONL")
    sprint3_readiness.add_argument("--manifest", type=Path, required=True, help="frozen benchmark manifest")
    sprint3_readiness.add_argument("--output", type=Path, required=True, help="new Sprint 3 readiness JSON report")
    ...
    if args.command == "build-external-circuit-corpus":
        try:
            manifest = build_external_circuit_corpus(
                args.output, {"serv": args.serv_source, "picorv32": args.picorv32_source, "rtl-riscv32": args.rtl_riscv32_source, "fazyrv": args.fazyrv_source, "learn-fpga": args.learn_fpga_source, "fma-rtl": args.fma_rtl_source}
            )
        except (OSError, ValueError, subprocess.CalledProcessError) as error:
    ...
    if args.command == "circuit-corpus-readiness":
        ...
        return 0
    if args.command == "sprint3-corpus-readiness":
        try:
            report = write_sprint3_corpus_readiness(args.source, args.manifest, args.output)
        except ValueError as error:
            print(f"Sprint 3 readiness failed: {error}")
            return 2
        print(f"Sprint 3 corpus ready for training: {report['ready_for_training']}")
        return 0
